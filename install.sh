...
# record and all three units do nothing.
...
# machine where nothing is paired reads an empty record, and all three units
# then do nothing at all.
#
# The first unit holds the boot and the shutdown, and the other two hold the
# sides of a sleep. That is one unit more than a stop of the first one on
# sleep.target, which was the earlier shape and left the lights on.
#
# The units run as the person who paired the devices, because the record and
# its tokens are in that person's home directory. A machine with no such user
...
if watcher_user_dirs; then
    ...
    write_unit "$SOURCE_DIR/server/$NAME-nanoleaf.service" \
        "$NANOLEAF_UNIT_PATH"
    write_unit "$SOURCE_DIR/server/$NAME-nanoleaf-sleep.service" \
        "$NANOLEAF_SLEEP_UNIT_PATH"
    ...
    systemctl enable "$NAME-nanoleaf-sleep.service" >/dev/null 2>&1 \
        || warn "could not enable $NAME-nanoleaf-sleep.service"
